...
function sign(role, extra) {
  const data = Object.assign({ exp: Date.now() + TTL_MS, role: role || 'admin' }, extra || {});
  const payload = Buffer.from(JSON.stringify(data)).toString('base64url');
  const sig = crypto.createHmac('sha256', SECRET).update(payload).digest('base64url');
  return `${payload}.${sig}`;
}

// Returns the decoded payload ({ exp, role, uid? }) on success, or null.
...
// Restrict a route to specific roles, e.g. requireRole('admin','hotel').
...
module.exports = {
  sign, verify, middleware, requireWrite, requireRole,
