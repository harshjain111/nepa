...
async function listRedemptions(opts = {}) {
  ...
  if (USE_SUPABASE) {
    const c = supabase.getClient();
    let q = c.from('meal_redemptions')
      .select('id, redeemedAt:redeemed_at, redeemedBy:redeemed_by, mealId:meal_id, ' +
        'registration:registrations(fullName:full_name, regId:reg_id, organization, mobile), ' +
        'meal:meals(name, mealDay:meal_day)')
      .order('redeemed_at', { ascending: false });
    ...
    return (data || []).map((r) => ({
      ...
      mealName: r.meal && r.meal.name, mealDay: r.meal && r.meal.mealDay,
    }));
  }
  ...
  return readJson(REDEEM_FILE)
    ...
    .map((x) => {
      ...
      return {
        ...
        mealName: m.name, mealDay: m.mealDay,
      };
    });
}
