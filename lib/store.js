...
const REG_SELECT = REG_SELECT_BASE +
  ', gstNumber:gst_number, designation, city, source, qrToken:qr_token, cardPrintedAt:card_printed_at, partyId:party_id, registeredBy:registered_by';
...
async function addRegistration(data) {
  if (USE_SUPABASE) {
    ...
    const row = {
      ...
      source: data.source || 'web', party_id: data.partyId || null, registered_by: data.registeredBy || null,
    ...
    if (error && isMissingCol(error)) {
      delete row.gst_number; delete row.designation; delete row.city; delete row.source; delete row.party_id; delete row.registered_by;
...
async function redeemMeal({ token, regId, mealId, by }) {
  ...
  if (USE_SUPABASE) {
    ...
    let q = c.from('registrations')
      .select('id, regId:reg_id, fullName:full_name, organization, designation, city, mobile, feeType:fee_type')
    ...
    // Exhibitors get a pass but NO meals / lunch.
    if (reg.feeType === 'Exhibitor') return { status: 'exhibitor', registrant: scan(reg), meal };
  ...
  if (!reg) return { status: 'notfound' };
  // Exhibitors get a pass but NO meals / lunch.
  if (reg.feeType === 'Exhibitor' || reg.source === 'exhibitor') return { status: 'exhibitor', registrant: scan(reg), meal };
...
/* ============================================================
   ...
   ============================================================ */
const PARTY_SELECT_BASE = 'id, createdAt:created_at, name, category, paidCount:paid_count, amount, notes, sort, active';
const PARTY_SELECT = PARTY_SELECT_BASE + ', originalPaid:original_paid, confirmedBy:confirmed_by, confirmedAt:confirmed_at';
...
async function listParties() {
  ...
  if (USE_SUPABASE) {
    ...
    if (error && isMissingCol(error)) {
      ({ data, error } = await supabase.getClient()
        .from('parties').select(PARTY_SELECT_BASE).order('sort', { ascending: true }).order('name', { ascending: true }));
    }
...
async function getParty(id) {
  const counts = await partyFilledCounts();
  if (USE_SUPABASE) {
    let { data, error } = await supabase.getClient().from('parties').select(PARTY_SELECT).eq('id', id).single();
    if (error && isMissingCol(error)) ({ data, error } = await supabase.getClient().from('parties').select(PARTY_SELECT_BASE).eq('id', id).single());
...
async function setPartyCount(id, count, by) {
  const n = Math.max(0, Math.floor(Number(count) || 0));
  if (USE_SUPABASE) {
    const patch = { paid_count: n, confirmed_by: by || null, confirmed_at: new Date().toISOString() };
    let { data, error } = await supabase.getClient().from('parties').update(patch).eq('id', id).select(PARTY_SELECT).single();
    if (error && isMissingCol(error)) {
      ({ data, error } = await supabase.getClient().from('parties').update({ paid_count: n }).eq('id', id).select(PARTY_SELECT_BASE).single());
    }
    if (error) return isNotFound(error) ? null : Promise.reject(partyErr(error));
    return data;
  }
  ...
  if (!p) return null;
  p.paidCount = n; p.confirmedBy = by || null; p.confirmedAt = new Date().toISOString();
  writeJson(PARTIES_FILE, rows);
  return p;
}
...
async function importParties(rows) {
  const clean = (rows || []).map((r, i) => ({
    ...
    original_paid: r.originalPaid != null && r.originalPaid !== '' ? String(r.originalPaid).slice(0, 50)
      : ((r.paidCount == null || r.paidCount === '') ? null : String(r.paidCount)),
    sort: i,
    active: true,
  })).filter((r) => r.name);
  if (USE_SUPABASE) {
    const c = supabase.getClient();
    const del = await c.from('parties').delete().not('id', 'is', null);
    if (del.error) throw partyErr(del.error);
    let stripped = false;
    for (let i = 0; i < clean.length; i += 200) {
      let chunk = clean.slice(i, i + 200);
      if (stripped) chunk = chunk.map(({ original_paid, ...rest }) => rest); // eslint-disable-line no-unused-vars
      let { error } = await c.from('parties').insert(chunk);
      if (error && isMissingCol(error) && !stripped) {
        // original_paid column not migrated yet — retry this + rest without it
        stripped = true; i -= 200; continue;
      }
      if (error) throw partyErr(error);
    }
  ...
  const rec = clean.map((r) => ({
    id: crypto.randomUUID(), createdAt: new Date().toISOString(),
    name: r.name, category: r.category, paidCount: r.paid_count, amount: r.amount,
    notes: r.notes, originalPaid: r.original_paid, sort: r.sort, active: true,
  }));
  writeJson(PARTIES_FILE, rec);
  return { ok: true, count: rec.length };
}

const partyFullErr = (name, cap) => { const e = new Error(`All ${cap} paid pass(es) for ${name} are already used. To add more, contact the accountant.`); e.code = 'PARTY_FULL'; return e; };
const noCountErr = () => { const e = new Error('This party has no delegate count yet — contact the accountant.'); e.code = 'NO_COUNT'; return e; };
const noPartyErr = () => { const e = new Error('Party not found.'); e.code = 'NO_PARTY'; return e; };

// Add one delegate against a party (help desk). ATOMIC on Supabase via the
// claim_party_slot() function (locks the party row) so two devices can never
// over-fill. Falls back to a check-then-insert if the function isn't created
// yet (and in JSON/local single-process mode).
async function addPartyDelegate(partyId, data) {
  if (USE_SUPABASE) {
    const { data: rpc, error } = await supabase.getClient().rpc('claim_party_slot', {
      p_party_id: partyId, p_full_name: data.fullName, p_org: data.organization || '',
      p_mobile: data.mobile || '', p_designation: data.designation || '', p_by: data.registeredBy || '',
    });
    if (!error) {
      const r = Array.isArray(rpc) ? rpc[0] : rpc;
      return { id: r.id, regId: r.reg_id, qrToken: r.qr_token, fullName: data.fullName };
    }
    const msg = String(error.message || '');
    if (/PARTY_FULL/.test(msg)) { const p = await getParty(partyId).catch(() => null); throw partyFullErr(p ? p.name : 'this party', p ? p.paidCount : ''); }
    if (/NO_COUNT/.test(msg)) throw noCountErr();
    if (/NO_PARTY/.test(msg)) throw noPartyErr();
    // Function not deployed yet? -> fall through to the non-atomic path.
    const missingFn = error.code === 'PGRST202' || /claim_party_slot|Could not find the function|does not exist/i.test(msg);
    if (!missingFn) throw partyErr(error);
  }
  // Non-atomic fallback (JSON mode, or Supabase before the migration).
  const party = await getParty(partyId);
  if (!party) throw noPartyErr();
  if (party.paidCount == null) throw noCountErr();
  const counts = await partyFilledCounts();
  if ((counts[party.id] || 0) >= party.paidCount) throw partyFullErr(party.name, party.paidCount);
  const reg = await addRegistration({
    ...
    gstNumber: null, nepaMember: false, feeType: 'Prepaid', delegateFee: 0, membershipFee: 0,
    subtotal: 0, gstRate: 0, gstAmount: 0, totalAmount: 0, paymentMethod: 'Prepaid',
