...
async function redeemMeal({ token, regId, mealId, by }) {
  ...
  if (USE_SUPABASE) {
    ...
    // Events allow unlimited re-entry (a person can enter the hall many times);
    // meals enforce max_per_person (usually once).
    if (meal.kind !== 'event' && used >= (meal.maxPerPerson || 1)) {
    ...
    return { status: 'ok', registrant: scan(reg), meal, used: used + 1, max: meal.maxPerPerson || 1, reentry: meal.kind === 'event' && used > 0 };
  }
  // JSON mode
  ...
  if (meal.kind !== 'event' && prior.length >= (meal.maxPerPerson || 1)) {
  ...
  return { status: 'ok', registrant: scan(reg), meal, used: prior.length + 1, max: meal.maxPerPerson || 1, reentry: meal.kind === 'event' && prior.length > 0 };
}
