...
// gst_number is a newer optional column; the *_BASE variant omits it so the app
// degrades gracefully if the column hasn't been added to the live DB yet.
const REG_SELECT = REG_SELECT_BASE + ', gstNumber:gst_number';
...
async function addRegistration(data) {
  if (USE_SUPABASE) {
    ...
    const sel = 'regId:reg_id, totalAmount:total_amount, feeType:fee_type, fullName:full_name';
    let { data: ins, error } = await supabase.getClient()
      .from('registrations').insert(row).select(sel).single();
    // Graceful degradation: if the gst_number column hasn't been added yet, retry without it.
    if (error && isMissingCol(error)) {
      delete row.gst_number;
